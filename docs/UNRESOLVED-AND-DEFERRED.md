...
Alpha Slices 1–24 supersede the bootstrap's implementation prohibition only for the explicitly authorized application foundation, Core Archetype foundation/shared accounting, zero-net controlled Attribute/existing-Skill adjustments, and bounded same-XP swaps among already-audited safe Skill instances, Point Buy v0.1, the narrow Life Modules v0.15/Final Touches/84-item-equipment scope, static Public Alpha readiness, GitHub Pages deployment configuration, deployment verification, and public wording cleanup recorded above. Slices 19–24 add no rules or equipment data. These slices do not authorize broad Archetype editing, full Skill catalog management, broader Life Module content, a full equipment catalog, active item effects, true finalization, account/login/cloud save, backend services, deployment to another provider, resolution of open rules questions, PDF export, playable-sheet runtime behavior, post-Beta publications, or progression through any planetary rollout. Beta 1 remains a future milestone requiring completed Core + Companion character creation and PDF export; v1.0 remains later.
