...
- Resolution of the printed Archetype conflicts cataloged in `archetype-sheet-cross-check.md` unless an authoritative correction or explicit project interpretation is established. Slice 25 preserves the implementation and records prose/back-sheet differences without selecting a governing source.
...
- Broad Life Modules Workflow/UI Review remains deferred. Alpha Slice 26 resolves the queued Stage 0 implicit-context issue; Slice 27 adds safe selectors for currently known pending choices and distinguishes current blockers from final-validation warnings. Neither slice implements arbitrary subskill creation, a full Skill/Trait catalog, or broader workflow redesign.
...
Alpha Slices 1–27 supersede the bootstrap's implementation prohibition only for the explicitly authorized application foundation, Core Archetype foundation/shared accounting, zero-net controlled Attribute/existing-Skill adjustments, bounded same-XP swaps among already-audited safe Skill instances, the Slice 25 audit, Point Buy v0.1, the narrow Life Modules v0.15/Final Touches/84-item-equipment scope, the Slice 26 Stage 0 affiliation-flow hotfix, the Slice 27 pending-award/progression hotfix, static Public Alpha readiness, GitHub Pages deployment configuration, deployment verification, and public wording cleanup recorded above. Slices 19–27 add no rules or equipment data. These slices do not authorize broad Archetype editing, full Skill catalog management, broader Life Module content or workflow redesign, a full equipment catalog, active item effects, true finalization, account/login/cloud save, backend services, deployment to another provider, resolution of open rules questions, PDF export, playable-sheet runtime behavior, post-Beta publications, or progression through any planetary rollout. Beta 1 remains a future milestone requiring completed Core + Companion character creation and PDF export; v1.0 remains later.
