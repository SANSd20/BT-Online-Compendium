...
- Life Module catalog expansion beyond the eight audited entries implemented through Alpha Slice 16: universal Stage 0, Capellan Confederation/Capellan Commonality, Stage 1 Blue Collar and Back Woods, Stage 2 Back Woods and High School, Stage 3 Technical College, and Stage 4 Agitator.
...
- Negative-Trait XP purchase execution, equipment catalog expansion beyond the current 75 items, full era/market interpretation of the printed Availability triplet, comprehensive affiliation-code mapping, heavy/combat Vehicle Trait workflow, ammo/reload accounting, power consumption/recharging, communications/network behavior, sensor detection, recording/playback behavior, consumable-use tracking, movement/falling automation, armor/equipment condition, repair-job automation, healing/fatigue/addiction effects, health/combat state, true final character locking, ready-for-play state, and PDF export. Slice 16 completes the supplied legacy raw-rating backfill while preserving item rules as inert metadata, rejecting runtime-like inventory state, and retaining manual fallback after `ready-for-final-touches`.
