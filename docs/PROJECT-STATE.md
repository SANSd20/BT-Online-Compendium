...
| Beta 1 milestone | Future | Requires completed Core + Companion character creation and PDF export; not reached by Alpha Slice 24 |
...
Implementation resume point: **Alpha Slice 25 — Archetype Final Touches Handoff**, contingent on separate authorization to extend the existing Final Touches entry path beyond Life Modules. Do not add full Skill-catalog selection, arbitrary new Skills, specialties, Trait adjustments, GM override, unbalanced completion, campaign buy-up handling, rules/catalog content, or equipment data without separate authorization.
