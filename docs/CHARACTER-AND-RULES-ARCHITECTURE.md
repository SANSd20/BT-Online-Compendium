...
## Alpha Slice 14 implementation boundary

The implemented foundation provides typed Character and rules models, one shared character factory, structural validation, a versioned save envelope, browser-local persistence, JSON import/export, Archetype v0.1, Point Buy v0.1, and Life Modules v0.11 with Final Touches, a 75-item audited equipment catalog, affiliation-aware access review, and manual inventory fallback. The project remains Alpha; Beta 1 requires completed Core + Companion character creation and PDF export.
...
Slice 14 inventory accepts 75 current audited catalog items or manual entries. Batch entries preserve the exact printed rating, parsed three-code Availability triplet, and supplied hand-audited normalized rating. Normalized Tech and Legality must match the raw endpoints; normalized Availability must occur somewhere in the triplet. No universal position or era meaning is inferred. Medical Kit, Medipatch, and Stimpatch retain their stable IDs while their current definitions remain promoted from example-backed records to audited page-313 records. Purchases snapshot raw, normalized, and rules metadata, so later catalog changes do not silently rewrite saved inventory. Communications, sensor, power, recharger, visibility, consumable, and movement details remain metadata only. Manual entries continue to require full single ratings.
