...
## Alpha Slice 26 implementation boundary

The implemented foundation provides typed Character and rules models, one shared character factory, structural validation, a versioned save envelope, browser-local persistence, JSON import/export, an Archetype foundation with shared XP accounting, controlled level adjustments, and bounded same-XP Skill swaps, Point Buy v0.1, and Life Modules v0.15 with Final Touches, an unchanged 84-item audited equipment catalog, affiliation-aware access review, and manual inventory fallback. Alpha Slice 26 fixes only the Stage 0 Universal affiliation flow: Universal is not treated as an affiliation, context and affiliated language begin unselected, the engine requires a matching explicit context, and older Alpha saves infer the only compatible context during import. No new Life Module, source-package, rules, or catalog data is added; Stage 1–4, Archetype, Point Buy, and Final Touches behavior remain unchanged. The project remains Alpha; Beta 1 requires completed Core + Companion character creation and PDF export.
