...
## Alpha Slice 13 implementation boundary

The implemented foundation provides typed Character and rules models, one shared character factory, structural validation, a versioned save envelope, browser-local persistence, JSON import/export, Archetype v0.1, Point Buy v0.1, and Life Modules v0.10 with Final Touches, a 55-item audited equipment catalog, affiliation-aware access review, and manual inventory fallback. The project remains Alpha; Beta 1 requires completed Core + Companion character creation and PDF export.
...
Slice 13 inventory accepts 55 current audited catalog items or manual entries. Batch entries preserve the exact printed rating, parsed three-code Availability triplet, and supplied hand-audited normalized rating. Normalized Tech and Legality must match the raw endpoints; normalized Availability must occur somewhere in the triplet. No universal position or era meaning is inferred. Medical Kit, Medipatch, and Stimpatch retain their stable IDs while their current definitions are promoted from example-backed records to audited page-313 records. Purchases snapshot raw, normalized, and rules metadata, so characters created before that promotion retain their historical snapshot rather than being silently rewritten. Manual entries continue to require full single ratings.
