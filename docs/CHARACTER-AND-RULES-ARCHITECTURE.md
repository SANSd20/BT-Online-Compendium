...
## Alpha Slice 16 implementation boundary

The implemented foundation provides typed Character and rules models, one shared character factory, structural validation, a versioned save envelope, browser-local persistence, JSON import/export, Archetype v0.1, Point Buy v0.1, and Life Modules v0.13 with Final Touches, a reconciled 75-item audited equipment catalog, affiliation-aware access review, and manual inventory fallback. The project remains Alpha; Beta 1 requires completed Core + Companion character creation and PDF export.
...
Slice 16 inventory accepts the same 75 current audited catalog items or manual entries; no equipment or stable IDs were added or changed. The 14 formerly normalized-only Slice 11 records now preserve their supplied exact printed ratings and three-code Availability triplets, so all 75 current records carry `preserved` raw-rating status. Normalized Tech and Legality must match the raw endpoints and normalized Availability must occur somewhere in the triplet; no universal position or era meaning is inferred. Version-2 purchase snapshots preserve name, cost, category, source, affiliation, ratings, metadata, and notes, while pre-backfill normalized-only and example-backed snapshots remain import-compatible without being silently rewritten. Manual entries stay visibly non-catalog and retain their entered data. Item rules remain inert metadata and direct runtime-like inventory state is rejected.
