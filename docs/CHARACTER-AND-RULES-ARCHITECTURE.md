...
## Alpha Slice 27 implementation boundary

The implemented foundation provides typed Character and rules models, one shared character factory, structural validation, a versioned save envelope, browser-local persistence, JSON import/export, an Archetype foundation with shared XP accounting, controlled level adjustments, and bounded same-XP Skill swaps, Point Buy v0.1, and Life Modules v0.15 with Final Touches, an unchanged 84-item audited equipment catalog, affiliation-aware access review, and manual inventory fallback. Alpha Slice 27 adds selector metadata for already-supported pending awards: known language and `/Any` choices come from current Alpha data, while flexible awards select readable existing or focused Point Buy targets and retain stable IDs internally. New selection state validates against those safe options; older resolved choices remain import-compatible. Pending awards remain current-stage blockers, while outstanding prerequisites marked for final validation produce warnings and no longer suppress legal continuation. No new Life Module, source-package, rules, or equipment catalog data is added; Archetype, Point Buy, Final Touches, and Stage 1–4 rules remain unchanged. The project remains Alpha; Beta 1 requires completed Core + Companion character creation and PDF export.
...
Slice 24 adds a bounded `skill-swap` ledger variant. Its source must be an unchanged Skill allocation in the selected foundation. Its replacement must be an exact Skill address already present in the existing audited Core Archetype definitions, must have the same shared Point Buy XP value and level as the source, must not duplicate a foundation or active replacement Skill, and must retain an explicit subskill where the known identity requires one. The derived safe-target registry rejects mixed parameterized/unparameterized identities, conflicting labels, specialties, Field Aptitude/nonstandard XP cases, and any under-specified subskill. A swap record preserves source and replacement snapshots, the source award identity, audited replacement source citation, player-choice provenance, timestamps, and optional note. Removal restores the exact source-backed Skill ledger entry.

Slice 25 does not extend the domain model. `docs/archetype-sheet-cross-check.md` records which printed fields match the implemented foundation, which back-sheet details are presentation or derived data absent from the model, and which conflicts require explicit governing-source decisions. In particular, the audit does not turn back-sheet movement, condition-monitor, armor/BAR, or weapon-combat presentation into authoritative package data. It also does not resolve Tanker Attribute XP, Elemental Attribute Links, Scout's Equipped cost, `REF+DEX` labels, name capitalization, or published package-total discrepancies.

Full Skill catalog selection, arbitrary new Skills, specialties, Trait adjustments, GM override, freeform unbalanced completion, and the remaining 500 XP / 5,000 XP campaign buy-up are not implemented. Attribute, Skill-level, and swap equality calculations reuse Point Buy cost functions, but the Archetype experience remains a source-package adjustment workflow rather than full Point Buy.
