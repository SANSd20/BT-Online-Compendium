...
**Alpha Slice 14 is implemented.** The React + TypeScript + Vite application includes Archetype v0.1, Point Buy v0.1, and Life Modules v0.11 on the shared Character/Rules model, with versioned JSON character files, browser-local persistence, and import/export.

All eight published Core archetypes can create, display, save, export, and import characters with source provenance. Point Buy v0.1 creates a Normal Human draft with a focused catalog. Life Modules v0.11 covers the narrow Stage 0–4 branch, final review, Final Touches, and a searchable 75-item audited Core equipment catalog with manual fallback. It preserves raw and normalized ratings and calculates affiliation-adjusted Owned/Issued access without conflating Wealth, C-bills, Equipped, or ownership.
...
- [`src/ui/`](src/ui/) — app shell and functional Archetype v0.1, Point Buy v0.1, and Life Modules v0.11 routes
...
Repository bootstrap and Alpha Slices 1–14 are complete. The implemented app is local-first and requires no server, database, or account.
...
The next recommended implementation slice, only when separately authorized and supplied with audited rules data, is **Alpha Slice 15 — continued audited Core equipment coverage or a focused catalog-review slice**. It should expand deliberately from the current 75 items without adding runtime ammunition, power, sensors, communications, consumables, movement, armor, health, heavy/combat Vehicle Trait handling, PDF export, or ready-for-play status. Planetary Data Foundation Rollout 1 remains designed but unimplemented and requires separate authorization.
